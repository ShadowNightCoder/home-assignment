...
app.post('/api/login-vulnerable', (req, res) => {
    // take BOTH username and password from the frontend
    ...
    db.get(query, (err, row) => {
        if (err) {
            ...
            res.status(200).json({ success: true, message: 'Login Successful (Vulnerable)!', user: row.username });
        } else {
...
// run server on port 3000 while my angular on 4200
const PORT = 3000;
app.listen(PORT, () => {
